...
function gerarImpressaoAtestado(pacienteNome, motivo, dias, horaInicio, horaFim) {
  ...
  setTimeout(() => {
    ...
    // (Modifique esta função na Lógica de Pacientes)
// (Substitua esta função na Lógica de Pacientes)
...
// (Substitua este bloco na Lógica de Pacientes)
if (atestadoForm) {
...
// Lógica da Página de Agendamentos (VERSÃO ATUALIZADA COM CORREÇÃO DE MODAL)
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form && tabela && selectPacientesForm && filtroDataEl && editModalElement) {
    ...
    const atualizarTabelaAgendamentos = () => {
      ...
      agendamentosFiltrados.sort((a, b) => a.hora.localeCompare(b.hora));
...
// Lógica da Página Financeiro (ATUALIZADA COM MODAL DE EDIÇÃO)
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form && tabela && selectPacientes && editModalElement) {
    ...
    // Remove a função global antiga (se houver)
    if (window.removerTransacao) {
      delete window.removerTransacao;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form && tabela) {
    ...
    const atualizarTabelaEstoque = () => {
      ...
      estoque.forEach((item, i) => {
        ...
        if (item.validade) {
          ...
          dataVal.setDate(dataVal.getDate() + 1); // Ajuste de fuso
          
          if (dataVal < hoje) {
            classeAlerta = "table-danger"; // Alerta de vencido
          }
          dataValidadeFormatada = dataVal.toLocaleDateString("pt-BR");
        }
        ...
        tr.innerHTML = `
          ...
          <td>
            <button class="btn btn-sm btn-info me-1">Editar</button>
            <button class="btn btn-sm btn-danger" onclick="removerItemEstoque(${i})">Excluir</button>
          </td>
        `;
        tbody.appendChild(tr);
      });
    ...
    window.removerItemEstoque = (index) => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if(estoqueBaixoEl) {
    const itensBaixos = estoque.filter(item => parseInt(item.quantidade) <= parseInt(item.estoqueMinimo)).length;
